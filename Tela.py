...
class TelaUsuarios(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.botao_delete = tk.Button(self.botao,
                                      ...
                                      command=lambda: (
                                          self.controller.banco.Delete(int(self.campo_delete.get())),
                                          controller.mostrar_tela(TelaDeletado)
                                      ))

        self.botao_delete.pack(pady=10)

        self.botao_update = tk.Button(self.botao,
                                      text="Atualizar Usuário",
                                      command=self.ir_para_update)

        self.botao_update.pack(pady=10)

        self.botao_telaInicial = tk.Button(self.botao,
                                           text="Cadastrar outro usuário",
                                           command=lambda: controller.mostrar_tela(TelaCadastro))

        self.botao_telaInicial.pack(pady=10)
    ...
    def guardar_idx_update(self):
        self.controller.idx = int(self.campo_delete.get())

    def ir_para_update(self):
        self.guardar_idx_update()
        self.controller.frames[TelaUpdate].carregar_usuario()
        self.controller.mostrar_tela(TelaUpdate)
...
class TelaDeletado(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.botao_erro = tk.Button(self,
                                    text="Voltar para lista de usuarios",
                                    command=lambda: (
                                        controller.frames[TelaUsuarios].atualizar_usuarios(),
                                        controller.mostrar_tela(TelaUsuarios)
                                    ))
...
class TelaUpdate(tk.Frame):
    ...
    def carregar_usuario(self):
        ...
        self.campo_nome.insert(0, usuario[1])
        self.campo_cpf.insert(0, usuario[2] if usuario[2] is not None else usuario[3])

        data_nascimento = usuario[4].split("-")
        if len(data_nascimento) == 3:
            self.dia.delete(0, tk.END)
            self.dia.insert(0, data_nascimento[0])
            self.mes.delete(0, tk.END)
            self.mes.insert(0, data_nascimento[1])
            self.ano.delete(0, tk.END)
            self.ano.insert(0, data_nascimento[2])

        self.campo_rua.insert(0, usuario[5] if usuario[5] is not None else "")
        self.campo_numero.insert(0, usuario[6] if usuario[6] is not None else "")
        self.campo_complemento.insert(0, usuario[7] if usuario[7] is not None else "")
        self.campo_bairro.insert(0, usuario[8] if usuario[8] is not None else "")
        self.campo_cidade.insert(0, usuario[9] if usuario[9] is not None else "")
        self.campo_uf.insert(0, usuario[10] if usuario[10] is not None else "")
        self.campo_cep.insert(0, usuario[11] if usuario[11] is not None else "")
        self.campo_contato.insert(0, usuario[12] if usuario[12] is not None else "")
        ...
        try:
            ...
            try:
                ...
                self.controller.banco.Update(
                    ...
                    self.campo_cep.get(),
                    self.campo_contato.get())
                self.controller.mostrar_tela(TelaAtualizadoSucesso)
            except:
                self.controller.mostrar_tela(TelaErroCEP)
...
class TelaAtualizadoSucesso(tk.Frame):
    def __init__(self, parent, controller):
        super().__init__(parent)
